    ...
    <div className="pt-44 px-[160px]">
      ...
      <div>
        ...
        <input
          ...
          name="zipcode"
          value={zipcode}
          onChange={(e) => setZipcode(e.target.value)} // Update state on change
      ...
      <div className="mt-20">
        ...
        {data?.results?.length > 0 ? (
          ...
          <div className="max-w-lg mx-auto my-10 p-6 bg-white shadow-lg rounded-lg">
            <h2 className="text-xl font-bold text-gray-800 mb-4">📍 Daftar Kode Pos</h2>
            {data?.results.map((item, index) => (
              <div key={index} className="border-b border-gray-300 pb-4 mb-4">
                <p className="text-gray-700">
                  <span className="font-semibold">Alamat:</span> {item.address1 + ' '},  {item.address2 + ' '},  {item.address3}
                </p>
                <p className="text-gray-500 text-sm">
                  <span className="font-semibold">Katakana:</span> {item.kana1 + ' '}, {item.kana2 + ' '}, {item.kana3}
                </p>
                <p className="text-gray-700">
                  <span className="font-semibold">Kode Prefektur:</span> {item.prefcode}
                </p>
                <p className="text-blue-600 font-semibold">📮 {item.zipcode}</p>
              </div>
            ))}
          </div>
          </div>
